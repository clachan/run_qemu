...
get_ovmf_binaries()
{
	...
	if ! [ -e "OVMF_CODE.fd" ] && ! [ -e "OVMF_VARS.fd" ]; then
		if ! [ -e "$_arg_vp_workspace/edk2/Build/OvmfX64/DEBUG_GCC5/FV/OVMF_CODE.fd" ] ||
		   ! [ -e "$_arg_vp_workspace/edk2/Build/OvmfX64/DEBUG_GCC5/FV/OVMF_VARS.fd" ]; then
			echo "OVMF_CODE.fd and/or OVMF_VARS.fd not found! Please build OVMF first!"
			return 1
		else
			cp $_arg_vp_workspace/edk2/Build/OvmfX64/DEBUG_GCC5/FV/OVMF_CODE.fd .
			cp $_arg_vp_workspace/edk2/Build/OvmfX64/DEBUG_GCC5/FV/OVMF_VARS.fd .
			return 0
		fi
